...
| **D12** | Velocity grid on `xenium-heimage-alignment`'s real inputs | **17×23** cells, from a source spanning 4100×5516 expanded units at `dv=250` | **48×66** — the cell count of a grid built over the *density's* 6000×8250 µm span at the same `dv` | on the notebook, relL2 on the affine **2.9e-04 → 1.8e+00**, warped density **1.9e-01 → 1.8e+00**, and the fit 7.3s → 216.7s at 100% GPU. `git bisect run` on the captured arguments names **`70069fc9`** exactly | **Open, and not a regression to revert.** Before `70069fc9`, `align_stalign_image` ignored each element's placement and built centred pixel axes, collapsing the same grid to **2×3** — near-rigid, hence near-affine, hence *accidentally* close to upstream. That commit made it read the real placement, which is correct: the axes round-trip is exact (0.0). So both grids were wrong and one was harmlessly so. **What is not yet established** is why the port's grid follows the density's extent rather than the source's — the two sides appear to disagree about which image moves, but the mapping was not proven, and mixed units alone do not reproduce it (`test_mixed_unit_velocity_grid_matches_upstream` passes over four axis configurations). `_core.lddmm` is byte-identical across the commit on these inputs, so whatever it is sits above the solver |
| **D11** | Rank-3 regulariser axes | the regularisation *energy* transforms two of the three spatial axes (`dim=(1,2)`, `:1504`) while the smoothing applied to that same energy's gradient spans all three (`dim=(1,2,3)`, `:1527`) | all three spatial axes in both places | upstream's regularisation energy reads **~5.4×** the three-axis value on the reference velocity field; squidpy matches the three-axis value to **< 1e-12** | **squidpy is right.** Kept, measured and pinned. Rank 3 only — at rank 2 the two readings coincide |
